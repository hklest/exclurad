...
# Update local checkout from a remote branch with safety guards.
#
# Usage examples:
...
if [[ "$BRANCH" != "$current_branch" && "$SWITCH_BRANCH" -ne 1 ]]; then
  echo "Refusing to switch branches implicitly." >&2
  echo "Current branch: $current_branch" >&2
  echo "Requested branch: $BRANCH" >&2
  echo "Re-run with --switch-branch if this is intentional." >&2
  exit 1
fi
...
if ! git diff --quiet || ! git diff --cached --quiet; then
  if [[ "$DO_STASH" -eq 1 ]]; then
    ...
    echo "Commit/stash first, or rerun without --no-stash." >&2
    exit 1
  fi
fi
...
backup_ref="backup/pre-update-${BRANCH}-$(date +%Y%m%d-%H%M%S)"
git branch "$backup_ref" >/dev/null 2>&1 || true
echo "Created safety backup branch: $backup_ref"
...
echo "If this update was not what you expected, recover with:"
echo "  git checkout $backup_ref"
echo "or inspect reflog:"
echo "  git reflog --date=local"
